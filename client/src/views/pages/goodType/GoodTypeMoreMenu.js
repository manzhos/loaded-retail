...
export default function GoodTypeMoreMenu({ id, onChange }) {
  ...
  const handleDelete = async (event) => {
    ...
    try {
      // console.log(`try deleting goodType #${id}`)
      await request(`${config.API_URL}api/delgoodtype/${id}`, 'GET', null, {
                  ...
                  <Grid container item spacing={2} xs={12} sm={10}>
                    <Grid item xs={12} sm={6}>
                      <TextField
                        ...
                        label="First Name"
                        autoFocus
                        defaultValue={goodType.firstname}
                      />
                    </Grid>
                    <Grid item xs={12} sm={6}>
                      <TextField
                        ...
                        defaultValue={goodType.lastname}
                      />
                    </Grid>
                    <Grid item xs={12}>
                      <TextField
                      ...
                      <FormControl sx={{ width: 1 }}>
                        ...
                        >
                          {roleList.map((item, key)=>{
                            return(
                              <MenuItem key={item.id} value={item.id}>{sentenceCase(item.goodTypetype)}</MenuItem>
                            )
                          })}
                        </Select>
                      </FormControl>

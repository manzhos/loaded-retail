...
export default function ShopMoreMenu({ id, onChange }) {
  ...
  const handleDelete = async (event) => {
    ...
    try {
      // console.log(`try deleting shop #${id}`)
      await request(`${config.API_URL}api/store/${id}`, 'DELETE', null, {
                      ...
                      <FormControl sx={{ width: 1 }}>
                        ...
                        >
                          {roleList.map((item, key)=>{
                            return(
                              <MenuItem key={item.id} value={item.id}>{sentenceCase(item.shoptype)}</MenuItem>
                            )
                          })}
                        </Select>
                      </FormControl>
